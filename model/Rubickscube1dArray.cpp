//
// created by Ganesh Pendem
//


#include "Rubickscube.h"

class Rubickscube1dArray : public Rubickscube {
private:

    //Given face index, row and coloumn then return its flattened index
...
public:
    ...
    bool operator==(const Rubickscube1dArray &r1) const {
        for (int i = 0; i < 54; i++) {
    ...
    Rubickscube1dArray &operator=(const Rubickscube1dArray &r1) {
        for (int i = 0; i < 54; i++) {
            cube[i] = r1.cube[i];
        }
        return *this;
...
struct Hash1d {
    size_t operator()(const Rubickscube1dArray &r1) const {
        string str = "";
